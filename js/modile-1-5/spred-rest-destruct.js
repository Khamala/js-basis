...
/*
// Очень часто вы будуте в свои функции получать непросто какой-то набор аргументов, а один объект         
// Назвывается это паттерн, т.е. прием программирования - Объект настроек. 
// В чем проблема функции с большим количеством аргументов? Вам нужно помнить, в каком порядке что передавать.

const showProfileInfo = function (userProfile) {
};

// И в вызов функции мы просто передаем какой-то объектс подписанными свойствами, например
// Это и называется паттерн - Объект настроек. 

// Когда он применяется? Когда у функции 3 и более аргументов.
...
/*
// И вот мы сейчас объект профиль передаем в нашу функцию
...
// Функция получает этот объект и может теперь работать со всеми его свойствами. И теперь мы хотим 
// деструктуризировать этот объект в локальные переменные, чтобы было удобно работать

const showProfileInfo = function (userProfile) {

    const { name, age, isOnline, friends, hobbies, games } = userProfile;
};

showProfileInfo(profile);

// Деструктуризация может быть проведена в самой функции, но также и прямо в подписи функциию
// Такое можно часто встретить. Т.е. прямо в место userProfile

// Это нам нужно будет часто. Практически все - плагины настраиваете, библиотеки подключаете,
// все настройки идут в виде объектов.
*/
